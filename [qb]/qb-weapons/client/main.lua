...
RegisterNetEvent('qb-weapons:client:AddAmmo', function(ammoType, _, itemData)
    ...
    local weaponName = QBCore.Shared.Weapons[weapon].name:lower()
    local clipSize = Config.MaxAmmo[weaponName] or maxAmmo
    ...
    QBCore.Functions.Progressbar('taking_bullets', Lang:t('info.loading_bullets'), Config.ReloadTime, false, true, {
        ...
        if not QBCore.Shared.Weapons[weapon] or QBCore.Shared.Weapons[weapon].ammotype ~= ammoType then
            return QBCore.Functions.Notify(Lang:t('error.wrong_ammo'), 'error')
        end

        local addAmount = math.min(clipSize - total, clipSize)
        if addAmount <= 0 then
            QBCore.Functions.Notify(Lang:t('error.max_ammo'), 'error')
            return
        end
        ...
        AddAmmoToPed(ped, weapon, clipSize)
        TriggerServerEvent('qb-weapons:server:UpdateWeaponAmmo', CurrentWeaponData, clipSize)
                                ...
                                if not data.RepairingData.Ready then
                                    ...
                                    DrawText3Ds(data.coords.x, data.coords.y, data.coords.z,
                                        Lang:t('info.repair_weapon_price',
                                            { value = Config.WeaponRepairCosts[WeaponClass] }))
                                    if IsControlJustPressed(0, 38) then
                                        QBCore.Functions.TriggerCallback('qb-weapons:server:RepairWeapon',
                                            function(HasMoney)
                                                if HasMoney then
                                                    CurrentWeaponData = {}
                                                end
